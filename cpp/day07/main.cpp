...
#undef X
 ...
 public:
  ...
  Gate(std::string name, Op op, std::array<std::string, 2> input)
    ...
    if (m_input[0].empty()) {
      ...
      if (m_input[1].empty()) {
        ...
        m_description =
            fmt::format("{} {} {}", m_input[0], opStr(op), m_input[1]);
      }
    }
  ...
  SignalStrength eval(std::unordered_map<std::string, Gate>* gates,
                      std::unordered_map<std::string, SignalStrength>* cache,
                      size_t depth) {
    if (DEBUG_OUTPUT) {
    ...
    switch (m_op) {
      case Op::SET:
        if (m_value1) {
          ...
          SignalStrength result =
              gates->at(m_input[0]).eval(gates, cache, depth + 1);
      ...
      case Op::RSHIFT: {
        SignalStrength result =
            gates->at(m_input[0]).eval(gates, cache, depth + 1) >> *m_bits;
        cache->emplace(m_name, result);
        if (DEBUG_OUTPUT) {
          fmt::print("{:{}}=> {}\n", "", depth * 2, result);
        }
      ...
      case Op::AND:
        if (m_value1) {
          SignalStrength result =
              *m_value1 & gates->at(m_input[1]).eval(gates, cache, depth + 1);
          cache->emplace(m_name, result);
          if (DEBUG_OUTPUT) {
            fmt::print("{:{}}=> {}\n", "", depth * 2, result);
          }
