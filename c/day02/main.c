...
int main(void) {
  ...
  while (true) {
    int rc = pcre2_match(linePattern, (PCRE2_SPTR)input, PCRE2_ZERO_TERMINATED,
                         offset, 0, matchData, NULL);
    ...
    result2 +=
        advent_minElement(ribbonLengths, ribbonLengths + ribbonLengthsCount) +
        l * w * h;
    free(ribbonLengths);
