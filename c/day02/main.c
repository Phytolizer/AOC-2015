...
int main(void)
{
    ...
    int errCode;
    size_t errOfs;
    pcre2_code* linePattern = pcre2_compile((PCRE2_SPTR) "^(\\d+)x(\\d+)x(\\d+)$", PCRE2_ZERO_TERMINATED,
    ...
    size_t offset = 0;
    pcre2_match_data* matchData = pcre2_match_data_create_from_pattern(linePattern, NULL);
    long result = 0;
    long result2 = 0;

    while (true)
    {
        int rc = pcre2_match(linePattern, (PCRE2_SPTR)input, PCRE2_ZERO_TERMINATED, offset, 0, matchData, NULL);
        ...
        long smallestSide = advent_minElement(sides, sides + sizeof(sides) / sizeof(long));
        ...
        for (int i = 0; i < sizeof(edges) / sizeof(long); ++i)
        {
            for (int j = 0; j < sizeof(edges) / sizeof(long); ++j)
            {
                ...
                {
                    ribbonLengthsCap *= 2;
                    long* newPtr = realloc(ribbonLengths, ribbonLengthsCap * sizeof(long));
                    if (newPtr == NULL)
                    {
        ...
        result2 += advent_minElement(ribbonLengths, ribbonLengths + ribbonLengthsCount) + l * w * h;
        free(ribbonLengths);
