...
int main(void) {
  ...
  int err;
  size_t errofs;
  pcre2_code* code = pcre2_compile((PCRE2_SPTR) "(\\w+) would (gain|lose) (\\d+) happiness units by sitting next to (\\w+)\\.",
  ...
  while (getline(&line, &line_length, fp) != -1) {
    ...
    int ret = pcre2_match(code, (PCRE2_SPTR)line, line_length, 0, 0, match_data,
                          NULL);
    if (ret < 0) {
  ...
  for (size_t i = 0; i < pairs.length; ++i) {
    ...
    assert(happiness_catalog_t_get(&catalog, pairs.data[i].a.s,
           ...
           "faulty logic");
    happiness_catalog_entry_t_insert(
        entry, pairs.data[i].b.s, pairs.data[i].b.l, pairs.data[i].happiness);
  }
  ...
  do {
    ...
    for (size_t i = 0; i < perm.length; ++i) {
      ...
      long* happiness;
      assert(
          happiness_catalog_entry_t_get(entry, name2.s, name2.l, &happiness) &&
          "faulty logic");
