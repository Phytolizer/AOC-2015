...
void Gate_init(Gate* g, const char* name, Op op, const char* inputA, const char* inputB, pcre2_code* wirePattern)
{
...
SignalStrength Gate_eval(Gate* g, GateMap* gates, SignalStrengthMap* cache, size_t depth)
{
    ...
    case OP_AND:
        ...
        else
        {
            ...
            Gate b;
            GateMap_get(gates, g->inputB, strlen(g->inputB), &b);
            result = Gate_eval(&a, gates, cache, depth + 1) & Gate_eval(&b, gates, cache, depth + 1);
            SignalStrengthMap_insert(cache, g->name, strlen(g->name), result);
#if DEBUG_OUTPUT
            printf("%*s=> %d (AND)\n", depth * 2, "", result);
#endif
...
bool parseLine(const char* line, pcre2_code* linePattern, pcre2_match_data* lineMatchData, pcre2_code* wirePattern,
               Gate* result)
{
    int rc = pcre2_match(linePattern, (PCRE2_SPTR)line, PCRE2_ZERO_TERMINATED, 0, 0, lineMatchData, 0);
    if (rc < 0)
    {
        return false;
    }
    ...
    else
    {
        ...
        for (char* word = strtok(inputs, " "); word != NULL; word = strtok(NULL, " "))
        {
        ...
        case 3:
            ...
            {
                Gate_init(result, name, OP_NOT, words.data[0], words.data[2], wirePattern);
            }
            else if (strcmp(words.data[1], "RSHIFT") == 0)
            {
                Gate_init(result, name, OP_RSHIFT, words.data[0], words.data[2], wirePattern);
            }
            else if (strcmp(words.data[1], "LSHIFT") == 0)
            {
                Gate_init(result, name, OP_LSHIFT, words.data[0], words.data[2], wirePattern);
            }
            else if (strcmp(words.data[1], "AND") == 0)
            {
                Gate_init(result, name, OP_AND, words.data[0], words.data[2], wirePattern);
            }
            else if (strcmp(words.data[1], "OR") == 0)
            {
                Gate_init(result, name, OP_OR, words.data[0], words.data[2], wirePattern);
            }
            else
            {
                return false;
...
int main(void)
{
    ...
    pcre2_match_data* lineMatchData = pcre2_match_data_create_from_pattern(linePattern, NULL);
