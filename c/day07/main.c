...
void Gate_init(Gate* g, const char* name, Op op, const char* inputA,
    ...
    case OP_LSHIFT:
    case OP_RSHIFT:
        assert(pcre2_match(wirePattern, (PCRE2_SPTR)inputA,
                           PCRE2_ZERO_TERMINATED, 0, 0, wireMatchData,
                           0) != PCRE2_ERROR_NOMATCH &&
               "Invalid SHIFT gate");
    ...
    case OP_NOT:
        assert(inputA == NULL &&
               pcre2_match(wirePattern, (PCRE2_SPTR)inputB,
                           PCRE2_ZERO_TERMINATED, 0, 0, wireMatchData,
                           0) != PCRE2_ERROR_NOMATCH &&
               "Invalid NOT gate");
        break;
    case OP_AND:
    case OP_OR:
        assert(pcre2_match(wirePattern, (PCRE2_SPTR)inputB,
                           PCRE2_ZERO_TERMINATED, 0, 0, wireMatchData,
                           0) != PCRE2_ERROR_NOMATCH &&
               "Invalid AND/OR gate");
        if (pcre2_match(wirePattern, (PCRE2_SPTR)inputA, PCRE2_ZERO_TERMINATED,
...
SignalStrength Gate_eval(Gate* g, GateMap* gates, SignalStrengthMap* cache,
    ...
    if (SignalStrengthMap_get(cache, g->name, strlen(g->name),
                              &cached_result)) {
#if DEBUG_OUTPUT
...
bool parseLine(const char* line, pcre2_code* linePattern,
               ...
               Gate* result) {
    int rc = pcre2_match(linePattern, (PCRE2_SPTR)line, PCRE2_ZERO_TERMINATED,
                         0, 0, lineMatchData, 0);
    if (rc < 0) {
        return false;
    } else if (rc == 0) {
        ...
        case 2:
            if (strcmp(words.data[0], "NOT") == 0) {
                Gate_init(result, name, OP_NOT, NULL, words.data[1],
                          wirePattern);
            } else if (strcmp(words.data[0], "RSHIFT") == 0) {
                Gate_init(result, name, OP_RSHIFT, NULL, words.data[1],
                          wirePattern);
            } else if (strcmp(words.data[0], "LSHIFT") == 0) {
                Gate_init(result, name, OP_LSHIFT, NULL, words.data[1],
                          wirePattern);
            } else if (strcmp(words.data[0], "AND") == 0) {
                Gate_init(result, name, OP_AND, NULL, words.data[1],
                          wirePattern);
            } else if (strcmp(words.data[0], "OR") == 0) {
                Gate_init(result, name, OP_OR, NULL, words.data[1],
                          wirePattern);
            } else {
                return false;
            }
...
int main(void) {
    ...
    pcre2_code* wirePattern =
        pcre2_compile((PCRE2_SPTR) "^[a-z]+$", PCRE2_ZERO_TERMINATED, 0,
                      &errCode, &errOfs, NULL);
    if (errCode < 0) {
        char message[120];
