...
static SetBucket* findBucket(SetBucket* data, size_t length, const void* value, size_t valueLength);
...
{
    if (s->capacity == 0 || (double)s->count / (double)s->capacity > SET_MAX_LOAD)
    {
        ...
        for (size_t i = 0; i < oldCapacity; ++i)
        {
            if (s->data[i].value != NULL)
            {
                SetBucket* bucket = findBucket(newData, s->capacity, s->data[i].value, s->data[i].valueLength);
...
static SetBucket* findBucket(SetBucket* data, size_t length, const void* value, size_t valueLength)
{
    ...
    while (data[h].value != NULL &&
           !(data[h].valueLength == valueLength && memcmp(data[h].value, value, valueLength) == 0))
    {
        h = (h + 1) % length;
    }
    return &data[h];
