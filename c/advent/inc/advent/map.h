...
#define MAP_PROTO_(MapType, ValueType)                                    \
  ...
  bool MapType##_insert(MapType* m, const void* key, size_t keyLength,    \
...
#define DEFINE_MAP(MapType, ValueType)                                        \
  ...
  MapType##Bucket* MapType##_findBucket_(MapType##Bucket* buckets,            \
                                         size_t bucketCount, const void* key, \
                                         size_t keyLength) {                  \
    if (bucketCount == 0) {                                                   \
      return NULL;                                                            \
    }                                                                         \
    ...
    while (buckets[h].key != NULL &&                                          \
           !(buckets[h].keyLen == keyLength &&                                \
             memcmp(buckets[h].key, key, keyLength) == 0)) {                  \
      h = (h + 1) % bucketCount;                                              \
    }                                                                         \
    return &buckets[h];                                                       \
  }                                                                           \
  bool MapType##_insert(MapType* m, const void* key, size_t keyLength,        \
    ...
    if (m->buckets == NULL) {                                                 \
    ...
    } else if ((double)m->entryCount / (double)m->bucketCount >               \
               MapType##_MAX_LOAD) {                                          \
      ...
      for (size_t i = 0; i < m->bucketCount / 2; i++) {                       \
        if (oldBuckets[i].key != NULL) {                                      \
          MapType##Bucket* b =                                                \
              MapType##_findBucket_(m->buckets, m->bucketCount,               \
                                    oldBuckets[i].key, oldBuckets[i].keyLen); \
