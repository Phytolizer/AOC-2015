...
#define DEFINE_MAP(MapType, ValueType)                                         \
    ...
    MapType##Bucket* MapType##_findBucket_(                                    \
        MapType##Bucket* buckets, size_t bucketCount, const void* key,         \
        size_t keyLength) {                                                    \
        if (bucketCount == 0) {                                                \
            return NULL;                                                       \
        }                                                                      \
        ...
        if (m->buckets == NULL) {                                              \
            ...
            for (size_t i = 0; i < m->bucketCount / 2; i++) {                  \
                if (oldBuckets[i].key != NULL) {                               \
                    MapType##Bucket* b = MapType##_findBucket_(                \
                        m->buckets, m->bucketCount, oldBuckets[i].key,         \
                        oldBuckets[i].keyLen);                                 \
