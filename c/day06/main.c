...
int main(void)
{
    ...
    pcre2_code* linePattern = pcre2_compile(
        (PCRE2_SPTR) "(?<action>turn on|turn off|toggle) (?<tlx>\\d+),(?<tly>\\d+) through (?<brx>\\d+),(?<bry>\\d+)",
    ...
    {
        int rc = pcre2_match(linePattern, (PCRE2_SPTR)line, PCRE2_ZERO_TERMINATED, 0, 0, matchData, NULL);
